...
| `src/data/kjv.js` | Full KJV text (generated — regenerate with `node tools/build-bible.js`, never edit) |
...
4. **Do not reproduce extended Bible passages in authored content** (questions,
   summaries, guide lines). Summarise and explain in the game's own words; short
   phrases only, always with a reference. The full KJV text in `src/data/kjv.js`
   is the one exception: it is the public-domain scripture itself, shipped
   verbatim as data, generated by `tools/build-bible.js` from
   scrollmapper/bible_databases — regenerate it, never hand-edit it.
...
- **Size:** shipping the full KJV inside `dist/bible-quest.html` grew the built file
  from ~400KB to ~4.6MB (`src/data/kjv.js` is ~4.2MB of public-domain text —
  31,102 verses as `KJV[bookId][chapter-1][verse-1]`). That is a deliberate
  trade for a single self-contained file that works offline from disk. Don't
  ship other large datasets without weighing the same trade.
